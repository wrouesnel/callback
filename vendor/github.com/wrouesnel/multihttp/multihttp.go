...
func NewClient(addr string) (*http.Client, error) {
	dialer, err := newDialer(addr, time.Duration(0))
	if err != nil {
		return nil, err
	}
...
// Initialize an HTTP client which connects to the provided socket address to
// service requests. The hostname in requests is parsed as a header only.
// This variant takes a time by which the request MUST complete.
func NewDeadlineClient(addr string, timeout time.Duration) (*http.Client, error) {
	dialer, err := newDialer(addr, timeout)
	if err != nil {
		return nil, err
	}
